!function(global, factory) {
    "object" == typeof exports && "undefined" != typeof module ? module.exports = factory() : "function" == typeof define && define.amd ? define(factory) : (global = global || self).Alpine = factory()
}(this, (function() {
    "use strict";
    ...
    function transitionIn(el, show, forceSkip=!1) {
        ...
        if (showAttr && showAttr.modifiers.includes("transition")) {
            ...
            attrs.length > 0 ? function(el, directives, showCallback) {
    ...
    function transitionOut(el, hide, forceSkip=!1) {
        ...
        if (showAttr && showAttr.modifiers.includes("transition")) {
            ...
            attrs.length > 0 ? function(el, directives, hideCallback) {
    ...
    function handleForDirective(component, el, expression, initialUpdate) {
        const {single: single, bunch: bunch, iterator1: iterator1, iterator2: iterator2} = function(expression) {
            ...
            iteratorMatch ? (res.single = single.replace(forIteratorRE, "").trim(),
            res.iterator1 = iteratorMatch[1].trim(),
            iteratorMatch[2] && (res.iterator2 = iteratorMatch[2].trim())) : res.single = single;
            return res
        }(expression);
    ...
    function handleAttributeBindingDirective(component, el, attrName, expression, extraVars) {
        ...
        if ("value" === attrName)
            if (void 0 === value && expression.match(/\./).length && (value = ""),
            "radio" === el.type)
                ...
                "SELECT" === el.tagName ? function(el, value) {
            ...
            ["disabled", "readonly", "required", "checked", "hidden", "selected"].includes(attrName) ? value ? el.setAttribute(attrName, "") : el.removeAttribute(attrName) : el.setAttribute(attrName, value)
    }
    function registerListener(component, el, event, modifiers, expression, extraVars={}) {
        if (modifiers.includes("away")) {
            const handler = e=>{
                el.contains(e.target) || el.offsetWidth < 1 && el.offsetHeight < 1 || (runListenerHandler(component, expression, e, extraVars),
              ...
              , handler = e=>{
                if (function(event) {
                    ...
                    if (0 === keyModifiers.length)
                        return !1;
                    if (1 === keyModifiers.length && keyModifiers[0] === keyToModifier(e.key))
                        return !1;
                    ...
                    selectedSystemKeyModifiers.length > 0) {
                        ...
                        e[`${modifier}Key`])).length === selectedSystemKeyModifiers.length && keyModifiers[0] === keyToModifier(e.key))
                            return !1
                    }
                    return !0
                }(e, modifiers))
                ...
                !1 === runListenerHandler(component, expression, e, extraVars) ? e.preventDefault() : modifiers.includes("once") && listenerTarget.removeEventListener(event, handler)
            }
            ;
    ...
    class Component {
        constructor(el, seedDataForCloning=null) {
            ...
            initExpression && !seedDataForCloning && (this.pauseReactivity = !0,
        ...
        walkAndSkipNestedComponents(el, callback, initializeComponentCallback=(()=>{}
            ...
            }(el, el=>el.hasAttribute("x-data") && !el.isSameNode(this.$el) ? (el.__x || initializeComponentCallback(el),
            !1) : callback(el))
        }
        initializeElements(rootEl, extraVars=(()=>{}
        )) {
            for (this.walkAndSkipNestedComponents(rootEl, el=>{
                if (void 0 !== el.__x_for_key)
                    return !1;
            ...
            this.executeAndClearRemainingShowDirectiveStack(); this.nextTickStack.length > 0; )
                this.nextTickStack.shift()()
        }
        initializeElement(el, extraVars) {
            el.hasAttribute("class") && getXAttrs(el).length > 0 && (el.__x_original_classes = el.getAttribute("class").split(" ")),
        ...
        resolveBoundAttributes(el, initialUpdate=!1, extraVars) {
            getXAttrs(el).forEach(({type: type, value: value, modifiers: modifiers, expression: expression})=>{
                ...
                case "bind":
                    ...
                    break;
                case "text":
                    void 0 === (output = this.evaluateReturnExpression(el, expression, extraVars)) && expression.match(/\./).length && (output = ""),
                    el.innerText = output;
                    break;
                case "html":
                ...
                case "show":
                    ...
                    !function(component, el, value, modifiers, initialUpdate=!1) {
                        ...
                        ;
                        modifiers.includes("immediate") ? handle(finish=>finish()) : (component.showDirectiveLastElement && !component.showDirectiveLastElement.contains(el) && component.executeAndClearRemainingShowDirectiveStack(),
                ...
                case "if":
                    ...
                    !function(el, expressionResult, initialUpdate) {
                        ...
                        if (expressionResult && !elementHasAlreadyBeenAdded) {
                            ...
                            !expressionResult && elementHasAlreadyBeenAdded && transitionOut(el.nextElementSibling, ()=>{
        ...
        evaluateCommandExpression(el, expression, extraVars=(()=>{}
            ...
            }(expression, this.$data, _objectSpread2({}, extraVars(), {
                $dispatch: this.getDispatchFunction(el)
            }))
        }
    ...
    return isTesting() || (window.Alpine = Alpine,
    window.Alpine.start()),
    Alpine
}
));
